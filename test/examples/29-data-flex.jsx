...
export default class DataFlexExample extends React.PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      items: [
        { key: "a", grow: 0, shrink: 1, order: 0 },
        { key: "b", grow: 1, shrink: 1, order: 1 },
        { key: "c", grow: 0, shrink: 1, order: 2 },
        { key: "d", grow: 0, shrink: 1, order: 3 },
        { key: "e", grow: 1, shrink: 1, order: 4 },
        { key: "f", grow: 0, shrink: 1, order: 5 }
      ]
    };
  }
  ...
  toggleGrow(key) {
    this.setState({
      items: this.state.items.map(item =>
        item.key === key ? { ...item, grow: item.grow === 0 ? 1 : 0 } : item
      )
    });
  }
  ...
  toggleShrink(key) {
    this.setState({
      items: this.state.items.map(item =>
        item.key === key ? { ...item, shrink: item.shrink === 1 ? 0 : 1 } : item
      )
    });
  }
            ...
            <tbody>
              {this.state.items.map(item => (
                <tr key={item.key} style={{ borderBottom: "1px solid #eee" }}>
                  <td style={{ padding: "10px", fontWeight: "bold" }}>{item.key.toUpperCase()}</td>
                  ...
                  <td style={{ padding: "10px" }}>
                    <button
                      onClick={() => this.toggleGrow(item.key)}
                      style={{
                        marginRight: "5px",
                        padding: "5px 10px",
                        cursor: "pointer",
                    ...
                    <button
                      onClick={() => this.toggleShrink(item.key)}
                      style={{
                        padding: "5px 10px",
                        cursor: "pointer",
                        background: "#28a745",
