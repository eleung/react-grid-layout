...
export function synchronizeFlexLayoutWithChildren(
  ...
  React.Children.forEach(children, (child: ReactElement<any>, i: number) => {
    ...
    if (exists && f == null) {
      ...
      if (f) {
        newLayout.push({ ...defaultFlexItem, ...f });
      } else {
...
export function childrenEqual(
  ...
  return deepEqual(
    React.Children.map(a, (c) => ({ key: c?.key, dataFlex: c?.props?.["data-flex"] })),
    React.Children.map(b, (c) => ({ key: c?.key, dataFlex: c?.props?.["data-flex"] }))
  );
}
